...
import { confirmAlert, confirmWarning, messageError } from "@/components";
...
import { acceptDesign } from "@/api/design";
  ...
  const { formik, loading, regField, regHandleSubmit } = useForm({
    onSubmit: (values) => {
      confirmAlert({
        ...
        yes: () => {
          dispatch(
            imageDesignActions.requestEditDesign({
              id: design.id,
    ...
    confirmWarning({
      ...
      yes: async () => {
        if (design.type === "3D") {
          ...
          const res = await acceptDesign(design.id);
          if (res.isSuccess) {
