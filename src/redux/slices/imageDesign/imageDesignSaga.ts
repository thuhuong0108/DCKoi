...
function* requestEditDesignWorker(
  action: PayloadAction<{ id: string; reason: string; idProject: string }>
) {
  try {
    const response = yield call(requestEditDesign, action.payload);
    if (response.isSuccess) {
      messageSuccess(response.message);
      yield put(imageDesignActions.fetchImageDesign(action.payload.id));
      yield put(designActions.fetchDesign(action.payload.idProject));
...
function* watchRequestEditDesign() {
  while (true) {
    const action = yield take(imageDesignActions.requestEditDesign);
    yield call(requestEditDesignWorker, action);
  }
}
...
export function* designImageSaga() {
  ...
  yield fork(watchRequestEditDesign);
}
